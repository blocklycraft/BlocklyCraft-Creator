...
export default {
  createProject (path, name, packagename) {
    ...
    // 解压种子项目
    fs.createReadStream(Path.join(__statics, '/seed.zip')).pipe(unzip.Extract({ path: path })) // eslint-disable-line no-undef
    ...
    fs.writeFileSync(path + '/info.json', JSON.stringify(projectInfo))
    return true
  },
  getProjectPath () {
    return project.path
  ...
  openProject (path) {
    ...
    // Now load project
    project.path = path
    const result = this.loadProjectInfo()
    if (result !== 'SUCCESS') {
      // fallback
      this.unloadProject()
    }
  ...
  loadProjectInfo () {
    const projectInfo = project.path + '/info.json'
    if (!fs.existsSync(projectInfo)) {
      logger.warn('File not found:' + projectInfo)
      return 'MISSING_INFO'
    }
  ...
  deleteBlock (name) {
    let index = 0
    for (const block of project.info.blocks) {
      if (block === name) {
  ...
  deleteCommand (command) {
    let index = 0
    for (const cmd of project.info.commands) {
  ...
  deletePermission (permission) {
    let index = 0
    for (const per of project.info.permissions) {
